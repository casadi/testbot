...
mkdir build
pushd build
../configure --prefix=/home/travis/ipopt-install --disable-shared ADD_FFLAGS=-fPIC ADD_CFLAGS=-fPIC ADD_CXXFLAGS=-fPIC --with-blas=BUILD --with-lapack=BUILD --with-mumps=BUILD --with-metis=BUILD --without-hsl --with-asl=BUILD
make
make install
popd && popd
tar -zcvf ipopt.tar.gz -C /home/travis/ipopt-install .
