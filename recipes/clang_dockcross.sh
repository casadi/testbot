...
if [ -z "$SETUP" ]; then
  ...
  build_env "sudo rpm --import ./RPM-GPG-KEY-WANdisco;sudo cp subversion.repo /etc/yum.repos.d/wandisco-svn.repo;sudo yum clean all;sudo yum install -y epel-release subversion libxml2-devel;cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=\"$mypwd/install\" ../llvm;make clang-tblgen install -j2 || make clang-tblgen install"
